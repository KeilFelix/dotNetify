    ...
    public class VMController : IDisposable
    {
        ...
        protected virtual string Serialize(object data)
        {
            try
            {
                ...
                string jsonOld = JsonConvert.SerializeObject(data, new JsonSerializerSettings { ContractResolver = new VMContractResolver(ignoredPropertyNames) });

                //JObject jObj = JObject.Parse(json);

                //jObj = SerializeDynamicProps(jObj, data);

                //var newto = jObj.ToString();
                ...
                //else
                //{
                //    data = SerializeHelper(new JObject(), data).ToString(Formatting.None);
                //}
        ...
        protected virtual JObject SerializeHelper(JObject jObject, object data)
        {
            ...
            foreach (PropertyDescriptor item in properties)
            {
                ...
                //JToken jToken;
                //if (!jObject.TryGetValue(item.Name, out jToken))
                //{
                //    jObject.Add(item.Name, JToken.Parse(Serialize(dataObj)));
                //}
            }
